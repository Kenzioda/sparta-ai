...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      const opencodeProvider = s.providers[ProviderV2.ID.make("opencode")]
      if (configured.length === 0 && opencodeProvider) {
        const [model] = sort(Object.values(opencodeProvider.models))
        if (model) return { providerID: opencodeProvider.id, modelID: model.id }
      }
