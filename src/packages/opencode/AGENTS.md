...
- Start it in `tmux` instead: `tmux new-session -d -s sparta-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt sparta-dev`.
- Stop the session explicitly when done: `tmux kill-session -t sparta-dev`.
