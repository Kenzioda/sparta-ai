...
# パッケージマネージャー
npm i -g sparta-ai@latest        # bun/pnpm/yarn でもOK
scoop install sparta             # Windows
choco install sparta             # Windows
brew install anomalyco/tap/sparta # macOS と Linux（推奨。常に最新）
brew install sparta              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S sparta            # Arch Linux (Stable)
paru -S sparta-bin               # Arch Linux (Latest from AUR)
mise use -g sparta               # どのOSでも
nix run nixpkgs#sparta           # または github:anomalyco/sparta で最新 dev ブランチ
```
...
S.P.A.R.T.A はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/sparta/releases) から直接ダウンロードするか、[S.P.A.R.T.A.ai/download](https://sparta.ai/download) を利用してください。
...
1. `$SPARTA_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.sparta/bin` - デフォルトのフォールバック
...
S.P.A.R.T.A には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://sparta.ai/docs/agents) の詳細はこちら。

### ドキュメント

S.P.A.R.T.A の設定については [**ドキュメント**](https://sparta.ai/docs) を参照してください。

### コントリビュート

S.P.A.R.T.A に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### S.P.A.R.T.A の上に構築する

S.P.A.R.T.A に関連するプロジェクトで、名前に "S.P.A.R.T.A"（例: "S.P.A.R.T.A-dashboard" や "S.P.A.R.T.A-mobile"）を含める場合は、そのプロジェクトが S.P.A.R.T.A チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/sparta) | [X.com](https://x.com/sparta)
